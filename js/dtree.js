...
// add a node  as sibling for the selected node 
//从dtree中找到已经选择的结点，然后插入到nodeId的结点后作为兄弟结点
dTree.prototype.addNodeAsSibling = function(dtree,nodeId){
	var selectedNodeId = this.selectedNode;
	var source = this.node(this.aNodes[selectedNodeId],selectedNodeId);
	var element = documnent.createElement(source);
	alert(element);
	var target = dtree.node(dtree.aNodes[nodeId],nodeId);
